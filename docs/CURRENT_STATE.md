...
- Stage 4 implementation PR #342 is in progress from implementation head lineage rooted at #341; it encodes only the frozen preregistration into `engine.py`, synthetic-only `qualification.py`, `test_engine.py`, and `IMPLEMENTATION_CONTRACT.json`. No CAPTURE-0002 raw artifact read or source-network fetch is authorized.
...
Complete #342 Stage-4 implementation handoff and exact-head standing CI, fix only implementation/mechanical defects without changing frozen Stage-3 science, then expected-head merge. Until #342 merges, formal completion remains 3/10. After that merge, Stage 4 becomes 4/10 and the next legal lifecycle step is formal Stage-5 nonhistorical qualification using synthetic fixtures only. Do not open CAPTURE-0002 raw values, add or substitute families/sources, refetch CAPTURE-0002, or change frozen thresholds/horizons/hypotheses.
