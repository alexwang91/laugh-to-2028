...
State = `OWNER-FIRST + DESIGN MERGED / STAGE 3 PREREGISTRATION OFFLINE FIRST-CAPTURE WIRING + EXECUTION-JOB CONTRACT MERGED / BYTE-PRESERVING WORKFLOW WIRING IN PROGRESS / FORMAL COMPLETION 2/10 / CONTROLLED ATTEMPT 0/1 / CONTROLLED READS 0`.
...
- Stage 3 execution-job contract merge `a2fb6612c8ec7e556585aafb9f759fa49834cfbb`.

OWNER-FIRST and DESIGN are merged. Stage 3 PREREGISTRATION remains active and incomplete. The source-identity precondition, first-capture boundary, offline capture engine/support plan and execution-job contract are merged. The current branch installs only the frozen one-shot job into the existing allowlisted `research-governance.yml`; exact compare against main shows the workflow diff is additions-only, with no mutation to the dormant OWNER-FIRST writers.

The installed job is gated to a later exact-main push containing `[0072_FIRST_CAPTURE_EXECUTE_V1]`, checks the fixed repository-wide raw/failure artifact name before source access, uses zero automatic retry and redirects, persists raw-or-failure state before any parse, creates a hash-bound durability receipt, finalizes only metadata after durability, and creates no Stage-8 marker. This workflow-wiring PR itself cannot invoke the live capture.

No live source endpoint has been called by this branch. No 0072 controlled funding/basis/history value has been exposed. Controlled reads remain `0`; attempt remains `0/1`. Production/signature/order authority remains zero. Liquidation intensity and external attention remain unqualified and cannot be substituted.

## Exact next step

Require fresh exact-head standing CI on `research/0072-first-capture-workflow-wiring-v1`. Merge only if the head is unchanged, open, mergeable, non-draft and every standing workflow succeeds. After merge, create a separate governance-only execution-request PR from exact main; only its all-green expected-head merge may use a merge commit title containing `[0072_FIRST_CAPTURE_EXECUTE_V1]` to invoke the one-shot metadata-only capture. If capture or any partial acquisition fails, the durable raw/failure artifact permanently blocks automatic refetch and requires manual governance reconciliation. If capture succeeds, inspect only the metadata artifact, register exact raw identities/support without opening raw payload content, and complete the full Stage 3 numerical/scientific preregistration. Formal completion remains 2/10 until that preregistration merges.
...
0072 = OWNER-FIRST + DESIGN MERGED / STAGE 3 OFFLINE WIRING + EXECUTION-JOB CONTRACT MERGED / WORKFLOW WIRING IN PROGRESS / FORMAL COMPLETION 2/10 / ATTEMPT 0/1 / CONTROLLED READS 0.
Phase 6 = NO CHANGE.