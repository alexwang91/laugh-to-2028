...
- Stage 5 nonhistorical qualification PR #343 is in progress; synthetic-only evidence blob `6275596a5db8c7d56e94db7b39394b95193a263b` records qualification PASS with controlled scientific-history reads 0, raw artifact reads 0, network fetches 0 and Stage-8 attempt consumed 0.
...
Complete #343 Stage-5 nonhistorical qualification handoff and exact-head standing CI. If the PR remains open, non-draft, mergeable and its head is unchanged, expected-head merge. Until #343 merges, formal completion remains 4/10. After merge, Stage 5 becomes 5/10 and the next legal lifecycle step is Stage 6 CONTROLLED-EXECUTION BOUNDARY. Stage 6 may freeze exact raw identities, read budgets, marker-before-read ordering, zero-network execution and exactly-once persistence, but may not open CAPTURE-0002 raw scientific values. Do not add or substitute families/sources, refetch CAPTURE-0002, or change frozen thresholds/horizons/hypotheses.
