...
if text.count(old) != 1:
    raise SystemExit(f'unexpected executive Stablecoin line count: {text.count(old)}')
...
if text.count(lineage_old) != 1:
    raise SystemExit('unexpected Stablecoin lineage paragraph')
text = text.replace(lineage_old, lineage_new, 1)

section_start = text.index('## STABLECOIN-LIQUIDITY-0001\n')
section_end = text.index('## Frozen product boundaries\n', section_start)
...
next_start = text.index('## Exact next action\n')
...
```
'''
text = text[:next_start] + next_replacement
...
for item in required:
    if item not in updated:
        raise SystemExit(f'missing terminal handoff item: {item}')
