...
# remain byte-identical at the git-blob level.
ALLOWED_EXACT_PATHS = {
...
def semantic_checks(root: Path) -> list[Check]:
    ...
    return [
        Check("canonical_directional_core", product.get("production", {}).get("model_version") == "BRRK-0011_FROZEN_RESEARCH_TARGET", repr(product.get("production", {}).get("model_version"))),
        ...
        Check("xrp_feature_only", "XRP remains feature-only" in str(p3_decision.get("decision", "")), str(p3_decision.get("decision", ""))),
        Check("primary_venue_hyperliquid", str(product.get("primary_venue", "")).lower() == "hyperliquid", repr(product.get("primary_venue"))),
        Check("daily_boundary_0000_utc", product.get("canonical_timezone") == "UTC" and product.get("daily_boundary_utc") == "00:00", f"{product.get('canonical_timezone')} {product.get('daily_boundary_utc')}"),
        Check("production_components_empty", decisions.get("production_authorized_components") == [], repr(decisions.get("production_authorized_components"))),
        ...
        Check("leverage_decision_still_1_0", "Current production gross cap remains 1.0" in str(leverage_decision.get("decision", "")), str(leverage_decision.get("decision", ""))),
        ...
        Check("phase6_elapsed_days_14", p6.get("acceptance", {}).get("live_shadow_observation", {}).get("minimum_elapsed_calendar_days") == 14, repr(p6.get("acceptance", {}).get("live_shadow_observation", {}).get("minimum_elapsed_calendar_days"))),
        Check("phase6_scheduled_decisions_10", p6.get("acceptance", {}).get("live_shadow_observation", {}).get("minimum_scheduled_decisions") == 10, repr(p6.get("acceptance", {}).get("live_shadow_observation", {}).get("minimum_scheduled_decisions"))),
        ...
        Check("phase8_trigger_absent", p8.get("status") == "PREREGISTERED_TRIGGER_ABSENT_NOT_RUN" and p8.get("trigger_present") is False, f"{p8.get('status')} trigger={p8.get('trigger_present')}"),
        Check("phase8_no_short_authority", p8.get("short_ready") is False and p8.get("production_authorized") is False and p8.get("first_real_short_authorized") is False, f"short_ready={p8.get('short_ready')} production={p8.get('production_authorized')} first_real={p8.get('first_real_short_authorized')}"),
    ]
...
def no_drift_snapshot(root: Path | None = None) -> dict[str, Any]:
    ...
    unauthorized = [path for path in paths if not path_is_allowed(path)]
    ...
    for path in BLOB_PARITY_PATHS:
        try:
            ...
            blob_checks.append({"path": path, "boundary_blob": before, "head_blob": after, "unchanged": before == after})
        except subprocess.CalledProcessError as exc:
            blob_checks.append({"path": path, "boundary_blob": None, "head_blob": None, "unchanged": False, "error": exc.stderr.strip()})

    semantics = semantic_checks(root)
    passed = not unauthorized and all(item.get("unchanged") is True for item in blob_checks) and all(check.passed for check in semantics)
    return {
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prove Program-Level Epistemic Governance v1 introduced no strategy/evidence/authority drift")
