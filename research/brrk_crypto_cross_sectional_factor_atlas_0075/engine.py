"""Deterministic, history-agnostic mechanics for BRRK 0075 Stage4.

This module encodes only frozen Stage3 transformations. It performs no I/O, no
network access, and contains no controlled historical payloads.
"""
...
def percentile(values: Sequence[float], q: float) -> float:
    """Linear percentile with deterministic interpolation, q in [0,1]."""
...
def fractional_ranks(values: Mapping[str, float]) -> dict[str, float]:
    """Average-rank ties mapped deterministically to [0,1]."""
    clean = {k: float(v) for k, v in values.items() if v is not None and isfinite(float(v))}
    if len(clean) < MIN_CROSS_SECTION:
        raise ValueError("insufficient cross section")
    ...
    while i < n:
        ...
        while j < n and ordered[j][1] == ordered[i][1]:
            j += 1
        avg_zero_based = (i + (j - 1)) / 2.0
        frac = 0.0 if n == 1 else avg_zero_based / (n - 1)
