...
RESULT_BRANCH = "research/0083-result-v1"
...
PINNED = {
    ...
    "research/brrk_sol_long_sideways_controller_integration_replacement_0083/qualification.py": "01b6554d0a943b98dbe6cade9e0f12acc577abd6",
...
def run_preflight(boundary_sha: str) -> dict:
    head = git("rev-parse", "HEAD")
    if head != boundary_sha:
        raise RuntimeError(f"FAIL exact merged-boundary mismatch: {head} != {boundary_sha}")
    bad = []
    ...
    if bad:
        raise RuntimeError(f"FAIL pinned identity mismatch: {bad}")
    existing = [name for name in RUNTIME if (HERE / name).exists()]
    if existing:
        raise RuntimeError(f"FAIL zero-result state violated: {existing}")
    remote_heads = git("ls-remote", "--heads", "origin", RESULT_BRANCH)
    if remote_heads:
        raise RuntimeError(f"FAIL result branch already exists: {RESULT_BRANCH}")
    return {
        "research_id": RID,
        ...
        "controlled_reads": {
            ...
            "MARKET_EVIDENCE": 0,
            "DTB3": 0,
            "0070_RESULT_CONTENT": 0,
        },
        "controlled_calls": {
            "market_loader": 0,
            "frozen_P02_prediction_reconstruction": 0,
            "cash_engine": 0,
            "validation_tuning": 0,
            "model_reselection": 0,
            "P02_retraining": 0,
...
def main() -> None:
    ...
    print(json.dumps(run_preflight(args.boundary_sha), indent=2, sort_keys=True))
