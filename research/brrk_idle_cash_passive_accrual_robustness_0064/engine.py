...
ANNUAL_SPREAD_FEE_BPS = (0, 50, 100, 150, 200)
PRIMARY = (0.50, 100)
CORE_YIELDS = (0.50, 0.75, 1.00)
CORE_FEES = (50, 100, 150)
MBB_BLOCK_LENGTH = 60
MBB_REPS = 4000
MBB_SEED = 640064
FROZEN_CELL_COUNT = 20
...
def causal_align_rates(strategy_dates: Sequence, source_dates: Sequence, source_values: Sequence[float]) -> np.ndarray:
...
def candidate_returns(
    ...
    if yield_realization not in YIELD_REALIZATIONS or annual_spread_fee_bps not in ANNUAL_SPREAD_FEE_BPS:
        raise MeasurementError("cell outside frozen grid")
    fee_daily = (float(annual_spread_fee_bps) / 10000.0) / YEAR_DAYS
    gross_carry = cash * float(yield_realization) * rf
    continuous_fee = cash * fee_daily
    net_cash_contribution = gross_carry - continuous_fee
    candidate = baseline + net_cash_contribution
    if not np.isfinite(candidate).all() or np.any(candidate <= -1.0):
        raise MeasurementError("invalid candidate return")
    return candidate, net_cash_contribution, gross_carry, continuous_fee
...
def mbb_lcb(d: Sequence[float], block_length: int = MBB_BLOCK_LENGTH, reps: int = MBB_REPS, seed: int = MBB_SEED) -> dict:
    x = _finite_1d(d, "d")
    n = len(x)
    ...
    return {"mu_obs": mu_obs, "q95": q95, "lcb": mu_obs - q95, "block_length": block_length, "replicates": reps, "seed": seed}


def cell_key(yield_realization: float, annual_spread_fee_bps: int) -> str:
    if yield_realization not in YIELD_REALIZATIONS or annual_spread_fee_bps not in ANNUAL_SPREAD_FEE_BPS:
        raise MeasurementError("cell outside frozen grid")
    return f"a{int(round(yield_realization * 100)):03d}_fee{annual_spread_fee_bps:03d}bps"
...
def evaluate(
    ...
    """Evaluate the frozen 20-cell passive-accrual measurement after external G0/G1 identity checks.

    This module performs no file or network I/O. The controlled runner owns Git/hash identity and exact baseline-anchor validation.
    """
    ...
    for a in YIELD_REALIZATIONS:
        for fee in ANNUAL_SPREAD_FEE_BPS:
            candidate, net_cash, gross_carry, continuous_fee = candidate_returns(baseline, idle_cash, rf_daily, a, fee)
            relative = relative_log_increment(candidate, baseline)
            key = cell_key(a, fee)
            cells[key] = {
                "yield_realization": a,
                "annual_spread_fee_bps": fee,
                ...
                "total_net_cash_contribution_return_units": float(np.sum(net_cash)),
                "total_gross_carry_return_units": float(np.sum(gross_carry)),
                "total_continuous_fee_return_units": float(np.sum(continuous_fee)),
    ...
    core_keys = [cell_key(a, fee) for a in CORE_YIELDS for fee in CORE_FEES]
    ...
    if not g2:
        classification = "FAIL_PASSIVE_CASH_PRIMARY_ECONOMICS"
    elif not g3:
        classification = "FAIL_PASSIVE_CASH_DRAWDOWN"
    elif not g4:
        classification = "FAIL_PASSIVE_CASH_TEMPORAL_ROBUSTNESS"
    elif not g5:
        classification = "FAIL_PASSIVE_CASH_DEPENDENCE_ROBUSTNESS"
    elif not g6:
        classification = "FAIL_PASSIVE_CASH_STRESS_ROBUSTNESS"
    else:
        classification = "PASS_PASSIVE_CASH_ACCRUAL_ROBUSTNESS"

    return {
        "candidate_cell_count": len(cells),
        "baseline": {"terminal_wealth": baseline_tw, "cagr": baseline_cagr, "max_drawdown": baseline_mdd},
