...
from research.brrk_leadership_4h_native_readiness_0054 import engine, run_once
...
class ControlledRunContractTests(unittest.TestCase):
    ...
    @staticmethod
    def _measurement(classification: str = "FAIL_4H_NATIVE_TRAINING_PRECISION_NOT_ESTABLISHED") -> dict:
        return {
    ...
    def test_evaluate_refuses_to_call_engine_before_attempt_marker(self) -> None:
    ...
    def test_staged_attempt_evaluate_finalize_calls_engine_exactly_once(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(paths["execution"].exists())
            ...
            self.assertEqual(result["classification"], "FAIL_4H_NATIVE_TRAINING_PRECISION_NOT_ESTABLISHED")
    ...
    def test_result_schema_rejects_predictive_metric_key(self) -> None:
    ...
    def test_runtime_artifact_names_are_frozen_but_directory_may_be_result_worktree(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            paths = self._paths(Path(tmp))
            ...
            with self.assertRaisesRegex(RuntimeError, "filenames differ"):
                run_once._verify_runtime_names(
                    Path(tmp) / "WRONG.json",
                    paths["execution"],
                    paths["attempt"],
                    paths["marker"],
