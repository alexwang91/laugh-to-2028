...
def main() -> None:
    ...
    if data.get("study_id") != "LEVERAGE-0040" or data.get("status") != "ONE_TIME_PREREGISTERED_STUDY_COMPLETE":
    ...
    if ev.get("runner_entrypoint") != "research/leverage_0040/run_leverage_0040_once_r1.py":
        raise SystemExit("R1 runner entrypoint evidence missing")
    if ev.get("preflight_corrections") != ["PREFLIGHT-RAW-TARGET-001", "PREFLIGHT-SESSION-TIMING-002"]:
    ...
    for cap in CAPS:
        ...
        if set(row.get("price_only_metrics_by_cost_bps", {})) != {"5.0", "10.0", "20.0", "50.0"}:
            raise SystemExit(f"cost grid mismatch {cap}")
        if cap != "1.00" and not {"final_research_pass", "broad_region_pass"}.issubset(row.get("gate", {})):
            raise SystemExit(f"gate bookkeeping missing {cap}")

    selection = data.get("selection", {})
    status = selection.get("status")
    if status not in {"NO_PROMOTION", "RESEARCH_PROMOTION_CANDIDATE_NOT_PRODUCTION_AUTHORIZED"} or selection.get("production_authorized") is not False:
    ...
    if status != "NO_PROMOTION":
        ...
        if selected not in {"1.10", "1.20", "1.30"} or matrix[selected]["gate"].get("final_research_pass") is not True:
