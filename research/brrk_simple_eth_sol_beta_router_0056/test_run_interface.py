...
from research.brrk_simple_eth_sol_beta_router_0056 import engine, run_once
...
class ControlledRunContractTests(unittest.TestCase):
    ...
    def _measurement(classification: str = "PASS_SIMPLE_BETA_ROUTER_ECONOMIC_ELIGIBILITY") -> dict:
        ...
        for cost in ("5", "10", "20"):
            ...
            for name, (wealth, turnover) in arm_values.items():
                metrics[cost][name] = {
                    ...
                    "cagr": wealth ** (engine.ANNUALIZATION_DAYS / engine.HELD_PERIODS) - 1.0,
        ...
        return {
            ...
            "gates": gates,
            "rm60_origin_count": engine.HELD_PERIODS,
            "target_count": engine.HELD_PERIODS,
            "targets": tuple(["ETH"] * engine.HELD_PERIODS),
            ...
            "bootstrap_5bps": {
                ...
                "blocks_per_replicate_before_truncation": engine.BOOTSTRAP_BLOCKS_PER_REPLICATE,
                "seed": engine.BOOTSTRAP_SEED,
            },
            "diagnostics": {
                "maximum_drawdown_5bps": {name: -0.20 for name in ("ROUTER", *engine.BENCHMARKS)},
                "router_total_executed_l1_turnover_5bps": 1.0,
                "router_switch_count": 0,
                "router_average_holding_duration_days": float(engine.HELD_PERIODS),
                "router_median_holding_duration_days": float(engine.HELD_PERIODS),
                "router_holding_spell_count": 1,
                "longest_underperformance_interval_days_vs_b_star_5bps": 0,
                "calendar_year_returns_5bps": {
                    name: {"2020": 0.01, "2021": 0.02} for name in ("ROUTER", *engine.BENCHMARKS)
                },
            },
    ...
    def test_evaluate_refuses_engine_before_attempt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()), mock.patch.object(
                engine, "evaluate_frozen_contract", return_value=self._measurement()
            ) as evaluate:
    ...
    def test_staged_attempt_evaluate_finalize_calls_engine_once(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()), mock.patch.object(
                source_engine, "load_frozen_market_evidence", return_value=frames
            ) as loader, mock.patch.object(
                engine, "evaluate_frozen_contract", return_value=self._measurement()
            ) as evaluate:
                result = run_once.evaluate_after_attempt(expected_head_sha="frozen-head", **paths)
                loader.assert_called_once_with(paths["market"])
            ...
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()), mock.patch.object(
                source_engine, "load_frozen_market_evidence", side_effect=AssertionError("finalize must never reload market")
            ) as loader, mock.patch.object(
                engine, "evaluate_frozen_contract", side_effect=AssertionError("finalize must never reevaluate")
            ) as evaluate:
    ...
    def test_finalize_rejects_tampered_result_without_remeasurement(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()):
                run_once.start_attempt(expected_head_sha="frozen-head", **paths)
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()), mock.patch.object(
                source_engine, "load_frozen_market_evidence", return_value={"ETH": object(), "SOL": object()}
            ), mock.patch.object(engine, "evaluate_frozen_contract", return_value=self._measurement()):
                run_once.evaluate_after_attempt(expected_head_sha="frozen-head", **paths)
            ...
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()), mock.patch.object(
                source_engine, "load_frozen_market_evidence", side_effect=AssertionError("finalize must never reload")
            ) as loader, mock.patch.object(
                engine, "evaluate_frozen_contract", side_effect=AssertionError("finalize must never reevaluate")
            ) as evaluate:
                with self.assertRaisesRegex(RuntimeError, "Primary result hash mismatch"):
                    run_once.finalize_marker_only(expected_head_sha="frozen-head", **paths)
                loader.assert_not_called()
                evaluate.assert_not_called()
                self.assertFalse(paths["marker"].exists())

    def test_protocol_error_becomes_frozen_invalid_execution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            paths = self._paths(Path(tmp))
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()):
                run_once.start_attempt(expected_head_sha="frozen-head", **paths)
            with mock.patch.object(run_once, "_verify_controlled_context", return_value=self._context()), mock.patch.object(
                ...
                engine, "evaluate_frozen_contract", side_effect=engine.RouterProtocolError("synthetic integrity failure")
            ) as evaluate:
                result = run_once.evaluate_after_attempt(expected_head_sha="frozen-head", **paths)
                evaluate.assert_called_once()
    ...
    def test_schema_rejects_predictive_metric_key(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "Forbidden non-preregistered metric"):
    ...
    def test_schema_rejects_classification_gate_mismatch(self) -> None:
        ...
        value = {
            ...
            "payload_sha256": schema["payload_sha256"],
            "execution_head_sha": "x",
            "classification": "FAIL_NO_SIMPLE_BETA_ROUTER_ECONOMIC_UPLIFT",
            "measurement": {**measurement, "classification": "FAIL_NO_SIMPLE_BETA_ROUTER_ECONOMIC_UPLIFT"},
            "authority": run_once._authority(),
        }
        with self.assertRaisesRegex(RuntimeError, "Classification does not match frozen G0-G4 precedence"):
            run_once.validate_result(value, schema)
