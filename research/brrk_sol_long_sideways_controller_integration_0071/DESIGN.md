# 0071 SOL Long-Sideways Controller Integration — DESIGN
...
## Purpose
0071 tests whether the locked 0070 P02 SOL `T4_LONG_SIDEWAYS` warning at exactly 10 sessions provides incremental economic risk-control value when translated into a bounded SOL long-or-cash controller under prospectively frozen costs and robustness tests.
...
## Immutable lineage
Required upstream anchors are closed 0064, 0069 and 0070. The 0070 immutable result merge is `174a30dc4950e7351f3a746edc8f581b8f12e6d3`, closeout merge is `d87607070cf03ccbbc318065f8c4c14ec6c6a50b`, and 0071 OWNER-FIRST merge is `4b8762f8046000c1bd414fb60b9232917251e579`.

Locked predictive input is `P02_RAW_ELASTIC_NET_LOGIT|SOL|T4_LONG_SIDEWAYS`, lead exactly 10 sessions. Closed reference anchors are PR-AUC `0.7974030713822858`, prevalence `0.45544554455445546`, PR-AUC lift `0.34195752682783037`, ROC-AUC `0.7913043478260869`, and 0070 median LOEO PR-lift retention `0.9823275624` with retention >=0.50 in `7/7` folds. These are lineage anchors, not new 0071 evidence.
...
The unique controlled attempt may occur only after every preceding stage has merged and exact merged-boundary zero-result preflight passes with attempt `0/1`, controlled reads zero and no result artifacts. A durable attempt marker must precede the first controlled content read. After attempt creation, no same-ID rerun, retune or rescue is permitted.
...
## Exact next stage
After exact-head standing CI passes and DESIGN merges, create a separate comprehensive PREREGISTRATION freezing exact controller parameters, benchmark/calendar alignment, costs, volatility/drawdown rules, turnover, metric formulas, bootstrap/DSR/PBO settings and seeds, concentration tests, attribution gates, classification thresholds, source identities/read budgets and exactly-once persistence rules before implementation or any controlled historical/economic read.
