...
def run_qualification() -> dict:
    ...
    regimes = {name: _classify_gate_fixture(name) for name in expected}

    # Purely synthetic mechanical fixture. Values are deliberately generated here;
    # no controlled historical/economic source is imported or read.
    n = 300
    ...
    for i in range(n):
        lag20.append(padded[i:i + 20].tolist())
    ...
    # Initial establishment turnover is charged from previous exposure=0.
    t_bench = turnover(expo["B00_FULLY_INVESTED_SOL"])
    establishment_ok = abs(float(t_bench[0]) - 1.0) <= 1e-12

    # Cost states must share the identical exposure path and differ only by one-way cost.
    ...
    expected_c1 = r0 - t0 * 0.001
    expected_c2 = r0 - t0 * 0.003
    cost_math_ok = np.allclose(r1, expected_c1, atol=1e-15, rtol=0.0) and np.allclose(r2, expected_c2, atol=1e-15, rtol=0.0)

    # Hysteresis must carry prior state strictly inside the 0.40-0.60 band.
    ...
    # Bootstrap generator must be deterministic, synchronized, bounded and exactly shaped.
