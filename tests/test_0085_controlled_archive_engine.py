    ...
    for asset_i, symbol in enumerate(("BTCUSDT", "ETHUSDT", "SOLUSDT"), start=1):
        for month, month_days in by_month.items():
            ...
            for idx, day in enumerate(month_days):
                ...
                # Deterministic positive path with varying daily increments so
                # realized volatility stays finite and positive.
...
def test_adapter_rejects_duplicate_asset_month_identity():
    ...
    # A duplicate month cannot be represented through the canonical filename
    # map itself; verify exact source naming fails closed instead of accepting
    # an alias that would create a second logical object.
